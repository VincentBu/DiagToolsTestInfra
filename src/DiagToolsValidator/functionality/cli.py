...
def release_invoker_resources(command_invoker: CommandInvoker):
    if command_invoker.redirect_out_err:
...
def run_command(args: Union[Iterable[str], str], wait_for_exit: bool = True, silent_run: bool = False, **kwargs):
    invoker = CommandInvoker(args, silent_run, **kwargs)
    if wait_for_exit:
